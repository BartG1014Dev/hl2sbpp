...
TOOL.Description = "Copy an entity (left) and paste it (left again)."
TOOL.CopiedData = nil

local function IsValidEntity(ent)
    return ent and not ent:IsPlayer()
end
    ...
    if trace:DidHitWorld() then
		...
		if data.phys and ent:VPhysicsGetObject() then
			...
			if data.phys.mass and phys.SetMass then
				pcall(function() phys:SetMass(data.phys.mass) end)
			end
			if data.phys.velocity and phys.SetVelocityInstantaneous then
				pcall(function() phys:SetVelocityInstantaneous(data.phys.velocity, data.phys.velocity) end)
			elseif data.phys.velocity and phys.SetVelocity then
				pcall(function() phys:SetVelocity(data.phys.velocity, data.phys.velocity) end)
			end
			phys:Wake()
		end
