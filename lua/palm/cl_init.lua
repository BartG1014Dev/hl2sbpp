    ...
    for k, v in pairs(tbl) do
        if k ~= "type" and k ~= "children" then
            ...
            elseif k == "w" or k == "width" then
                if panel.SetWidth then panel:SetWidth(v) end
                if panel.SetWide then panel:SetWide(v) end
            elseif k == "h" or k == "height" then
                if panel.SetHeight then panel:SetHeight(v) end
                if panel.SetTall then panel:SetTall(v) end
...
function IsValid(object)
    ...
    if object == INVALID_PANEL then return false end
    return true
end
