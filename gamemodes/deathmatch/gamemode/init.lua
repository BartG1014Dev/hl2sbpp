...
function GM:GetPlayerHandModel(player, handStr, soundType) end
...
function GM:GiveDefaultItems(pPlayer)
  ...
  if
    pPlayer:GetPlayerModelType() == PLAYER_SOUNDS_METROPOLICE
    or pPlayer:GetPlayerModelType() == PLAYER_SOUNDS_COMBINESOLDIER
  then
...
function GM:PlayerEntSelectSpawnPoint(pHL2MPPlayer) end
